...
weston_output_destroy(struct weston_output *output)
{
	...
	weston_colorspace_destroy(output->colorspace, 0);
		   ...
		   int32_t scale)
{
	...
	output->colorspace = c->srgb_output_colorspace;
		       ...
		       struct weston_config *config)
{
	...
	if (!weston_create_cms_interface(display, ec))
		return -1;
...
int main(int argc, char *argv[])
{
	...
	wl_list_init(&ec->input_colorspaces);
	wl_list_init(&ec->output_colorspaces);
	if (!weston_create_default_colorspaces(ec)) {
		ret = EXIT_FAILURE;
		goto out;
	}
...
out:
	...
	weston_colorspace_destroy(ec->srgb_output_colorspace, 1);
	weston_colorspace_destroy(ec->srgb_colorspace, 1);
	weston_colorspace_destroy(ec->blending_colorspace, 1);
